...
impl Grid {
    pub(crate) fn new(bounds: Bounds, cell_size: f32) -> Self {
    ...
    #[inline]
    pub(crate) fn for_each_candidate(&self, p: Vec3, mut f: impl FnMut(u32)) {
...
pub struct Fluid {
    ...
    pub pos: Vec<Vec3>,
    pub previous_pos: Vec<Vec3>,
    pub vel: Vec<Vec3>,
    /// Advected aeration indicator, generated by local velocity disorder.
    pub foam: Vec<f32>,
...
impl Fluid {
    ...
    pub fn fill_wave(&mut self, wave: crate::wave::Wave, swell: crate::wave::Swell) {
        ...
        for z in 0..counts.z {
            for y in 0..counts.y {
                for x in 0..counts.x {
                    let p =
                        b.min + (Vec3::new(x as f32, y as f32, z as f32) + Vec3::splat(0.5)) * d;
    ...
    /// Surface aeration follows relative motion, not absolute speed: a clean
    /// translating lip stays green, while impacts and fragmented jets whiten.
    fn update_foam(&mut self, dt: f32) {
        let pos = &self.pos;
        let vel = &self.vel;
        let starts = &self.neighbor_start;
        let neighbors = &self.neighbors;
        let h2 = self.kernels.h2;
        let speed_scale = (self.params.gravity.length() * self.params.spacing)
            .sqrt()
            .max(1.0);
        self.foam
            .par_iter_mut()
            .zip(self.spray.par_iter_mut())
            .enumerate()
            .for_each(|(i, (foam, spray))| {
                let mut count: f32 = 0.0;
                let mut disorder = 0.0;
                for &j in &neighbors[starts[i] as usize..starts[i + 1] as usize] {
                    if (pos[i] - pos[j as usize]).length_squared() < h2 {
                        count += 1.0;
                        disorder += (vel[i] - vel[j as usize]).length_squared();
                    }
                }
                let agitation = (disorder / count.max(1.0)).sqrt() / speed_scale;
                let exposed = ((28.0 - count) / 18.0).clamp(0.0, 1.0);
                let source = ((agitation - 0.30) * 1.8).clamp(0.0, 1.0) * exposed;
                *foam = (*foam * (-dt / 1.8).exp() + source * dt * 3.0).clamp(0.0, 1.0);
                *spray = ((10.0 - count) / 10.0).clamp(0.0, 1.0)
                    * (vel[i].length() / speed_scale - 0.6).clamp(0.0, 1.0);
            });
    }
...
mod tests {
    ...
    #[test]
    fn coherent_translation_does_not_generate_foam() {
        let mut fluid = Fluid::new(params());
        fluid.fill_block([6, 6, 6]);
        fluid.step(1.0 / 600.0);
        fluid.foam.fill(0.0);
        fluid.vel.fill(Vec3::new(900.0, 0.0, 0.0));
        fluid.update_foam(1.0 / 60.0);
    ...
    fn mouse_response() {
        ...
        println!(
            "{:>10} {:>8}  {:>10} {:>10}",
            "strength", "radius", "peak", "surface_up"
        );
